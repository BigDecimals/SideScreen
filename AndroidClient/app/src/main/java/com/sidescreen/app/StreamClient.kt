...
class StreamClient(
    ...
    // Pre-allocated buffer for touch/ping serialization to reduce GC churn
    // Safe to use without synchronization because all writes happen on the single-threaded touchScope
                ...
                while (isConnected) {
                    ...
                    when (type.toInt()) {
                        ...
                        5 -> { // Pong response — measure round-trip latency
                            val sentTime = java.lang.Long.reverseBytes(input.readLong()) // zero-allocation little-endian read
